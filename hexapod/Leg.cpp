...
bool Leg::setFootPosition(int16_t footPosX, int16_t footPosY, int16_t footPosZ, int8_t speed)
{
    ...
    //Serial.println("Angle diffs: hip=" + String(hipAngleDiff) + ", knee=" + String(kneeAngleDiff) + ", foot=" + String(footAngleDiff));
    ...
    //Serial.println("Largest angle diff: " + String(largestDiff));

    // Calculate servo speeds
    // If servos need to move differently adjust speed so all servos reach their target at the same time
    uint8_t hipSpeed = speed;
    uint8_t kneeSpeed = speed;
    uint8_t footSpeed = speed;
